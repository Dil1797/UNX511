...
int hardwareSim(void *data); // ✅ Added missing prototype
...
int register_device(void)
{
    ...
    if (!IS_ERR(threadPtr)) {
        ...
        printk(KERN_ERR "Hardware-Device: Failed to create kernel thread\n");
        return PTR_ERR(threadPtr);
    }
...
static ssize_t hardware_device_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
    ...
    if (copy_to_user(buf, buffer, bytes_read)) {
        printk(KERN_WARNING "Hardware-Device: Failed to copy data to user space\n");
        return -EFAULT;
    }

    printk(KERN_INFO "Hardware-Device: Sent buffer to user: %s\n", buffer);
    return bytes_read;
}
...
static long hardware_device_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    switch (cmd) {
        case HARDWARE_DEVICE_HALT:
            is_halt = true;
            printk(KERN_INFO "Hardware-Device: HALT command received\n");
            break;

        case HARDWARE_DEVICE_RESUME:
            is_halt = false;
            printk(KERN_INFO "Hardware-Device: RESUME command received\n");
            break;

        default:
            printk(KERN_WARNING "Hardware-Device: Unknown ioctl command: %u\n", cmd);
            return -EINVAL;
    }
